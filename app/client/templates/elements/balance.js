...
Template['elements_balance'].helpers({
    ...
    */
    'convertedBalance': function(){
        ...
        if (AITTools.getUnit() === 'noether') return 'infinite';

        if(balance){
            if(AITTools.getUnit() === 'usd' || AITTools.getUnit() === 'eur' || AITTools.getUnit() === 'gbp' || AITTools.getUnit() === 'brl')
                return AITTools.formatBalance(TemplateVar.get('balance'), '0,0.00');
            else if(AITTools.getUnit() === 'ether')
                return AITTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[0000000000000000]' : '0,0.00') );
            else if(AITTools.getUnit() === 'finney')
                return AITTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[00000000000000]' : '0,0.00'));
            else
                return AITTools.formatBalance(TemplateVar.get('balance'), '0,0.00[000000]');
        }
    },
    /**
