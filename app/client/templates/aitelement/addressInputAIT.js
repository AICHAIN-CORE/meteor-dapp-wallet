...
var ensAddress = 'ai314159265dd8dbb310642f98f50c066173c1259b';
...
@class [template] dapp_addressInputAIT
@constructor
*/

Template.dapp_addressInputAIT.onCreated(function() {
  var template = this;
...
Template.dapp_addressInputAIT.onRendered(function() {
  if (this.data) {
    this.$('input').trigger('change');
  }
});

Template.dapp_addressInputAIT.helpers({
  /**
  ...
  'address': function() {
    ...
    if (_.isString(address) && web3.isAddress(address)) {
        if (address.toLowerCase().substring(0, 2) === '0x') {
...
Template.dapp_addressInputAIT.events({
  /**
  ...
  'input input, keyup input': function(e, template) {
    ...
    // add 0x
    if (value.length > 38
            && value.indexOf('0x') === -1 && value.indexOf('ai') === -1
    ...
    if (web3.isAddress(value) || _.isEmpty(value)) {
      ...
      if (!_.isEmpty(value)) {
        if (value.toLowerCase().substring(0, 2) === '0x') {
            TemplateVar.set('value', 'ai' + value.replace('0x', ''));
        } else if (value.toLowerCase().substring(0, 2) === 'ai') {
            TemplateVar.set('value', 'ai' + value.replace('ai', ''));
        } else {
            TemplateVar.set('value', 'ai' + value.replace('0x', ''));
        }
