...
@class [template] dapp_selectAITGasPrice
@constructor
*/
...
var calculateGasInWei = function(template, gas, gasPrice, returnGasPrice){
    ...
    if(_.isUndefined(gas)) {
        console.warn('No gas provided for {{> dapp_selectAITGasPrice}}');
...
Template['dapp_selectAITGasPrice'].onCreated(function(){
...
Template['dapp_selectAITGasPrice'].helpers({
    /**
    ...
    */
    'fee': function(){
        if(_.isFinite(TemplateVar.get('feeMultiplicator')) && _.isFinite(this.gas)) {
            ...
            // return the fee
            return AITTools.formatBalance(calculateGasInWei(template, this.gas, this.gasPrice).toString(10), '0,0.[000000000000000000]', this.unit);
        }
    },
    /**
    ...
    */
    'unit': function(){
        var unit = this.unit || AITTools.getUnit();
        if(unit)
...
Template['dapp_selectAITGasPrice'].events({
    /**
